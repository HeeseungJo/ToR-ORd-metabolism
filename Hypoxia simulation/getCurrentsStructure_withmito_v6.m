...
function currents = getCurrentsStructure_withmito_v6(time, X, parameters, ignoreFirstSpikes)
...
currents.force_norm = zeros(nPoints, 1);
currents.force = zeros(nPoints, 1);
currents.VNO = zeros(nPoints, 1);
...
for iBeat = 1:size(time, 1)
    ...
    for j=1:size(X{iBeat},1)
       IsJs=modelNow(time{iBeat}(j),X{iBeat}(j,:),0, cellType, ICaL_Multiplier, ...
        ...
        currents.force_norm(i) = IsJs(37); 
        currents.force(i) = IsJs(38);
        currents.VNO(i) = IsJs(39);
