...
%%%%%%%%%%%%%%%%   INaK, IpCa, Jup with ATP dependent scalars %%%%%%%%%%%%%%%%%%%%%%
% Made Jup_cortassa 0 if negative



function output=model_Torord_withmito_v6_VnaCa_unscaled(t,X,flag_ode, cellType, ICaL_Multiplier, ...
...
ADP = 8 - ATPi; % 8 mM total ADP/ATP in cell from Cortassa 2006
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% INaK
INaK_cortassa_scalar = getINaK_ORd2011_cortassascalar(v, F, R, T, nai, nao, ki, ko, ATPi, ADP, celltype, INaK_Multiplier);
...
%%%%%%%%%%%%%%% calculate IpCa with Cortassa scalar f^ATP_pCa  %%%%%%%%%%%%%%%%%
GpCa=5e-04*IpCa_Multiplier;

IpCa=GpCa*cai/(0.0005+cai);


K_m1pCa = 0.012; % mM First ATP half saturation constant for sarcolemmal ca pump
K_m2pCa = 0.23; % mM second atp half saturation constant for sarcolemmal ca pump
K_ipCa =  1.0; % mM ADP inhibition constant for sarcolemmal ca pump

fpCa_ATP = (1 / (1 + ((K_m1pCa / ATPi) * (1 + (ADP/K_ipCa))))) + (1 / (1 + (K_m2pCa/ATPi)));

IpCa_cortassa_scalar = IpCa * fpCa_ATP;
...
[Jup_cortassa_scalar, Jleak] = getJup_ORd2011_cortassascalar(cai, cansr, fJupp, ATPi, ADP, celltype, Jup_Multiplier); %%%% cortassa scalar with Jup
...
dv=-(INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+INaCa_i+INaCa_ss+INaK_cortassa_scalar+INab+IKb+IpCa_cortassa_scalar+ICab+I_ClCa+I_Clbk+Istim);
...
dnai=-(ICaNa_i+INa+INaL+3.0*INaCa_i+3.0*INaK_cortassa_scalar+INab)*Acap/(F*vmyo)+JdiffNa*vss/vmyo;
dnass=-(ICaNa_ss+3.0*INaCa_ss)*Acap/(F*vss)-JdiffNa;

dki=-(ICaK_i+Ito+IKr+IKs+IK1+IKb+Istim-2.0*INaK_cortassa_scalar)*Acap/(F*vmyo)+JdiffK*vss/vmyo;
dkss=-(ICaK_ss)*Acap/(F*vss)-JdiffK;

Bcai=1.0/(1.0+cmdnmax*kmcmdn/(kmcmdn+cai)^2.0+trpnmax*kmtrpn/(kmtrpn+cai)^2.0);
dcai=Bcai*(-(ICaL_i + IpCa_cortassa_scalar+ICab-2.0*INaCa_i)*Acap/(2.0*F*vmyo)-Jup_cortassa_scalar*vnsr/vmyo+Jdiff*vss/vmyo);
...
dcansr=Jup_cortassa_scalar-Jtr*vjsr/vnsr;

Bcajsr=1.0/(1.0+csqnmax*kmcsqn/(kmcsqn+cajsr)^2.0);
dcajsr=Bcajsr*(Jtr-Jrel);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cortassa Mitochondrial Function

% changing state variable to be compatible 
Cai = cai;
Nai = nai;


%%%%%%%%% using cortassa equations with parameter values from cellml model
[dCam, dADPm, dDpsi, dSCoA, dNADH, dISOC, dAKG, dSucc, dFUM, dMAL, dOaa, dASP, dATPi, dATPi_cyto, dCrPi_mito, dCrPi_cyto, dP0, dP1, dP2, dP3, dN1, dLTRPNCa, dHTRPNCa, force_norm, force, VNO] ...
    = calculateF_mitene_full_v6_cellml_VnaCa_unscaled(Cam, ADPm, Dpsi, SCoA, NADH, ISOC, AKG, Succ, FUM, MAL, Oaa, ASP, ATPi, ATPi_cyto, CrPi_mito, CrPi_cyto, P0, P1, P2, P3, N1, LTRPNCa, HTRPNCa, Cai, Nai, Jup_cortassa_scalar, INaK_cortassa_scalar, IpCa_cortassa_scalar);
...
if flag_ode==1
    output=[dv dnai dnass dki dkss dcai dcass dcansr dcajsr dm dhp dh dj djp dmL dhL dhLp da diF diS dap diFp diSp,...
        ...
        dCam, dADPm, dDpsi, dSCoA, dNADH, dISOC, dAKG, dSucc, dFUM, dMAL, dOaa, dASP, dATPi, dATPi_cyto, dCrPi_mito, dCrPi_cyto, dP0, dP1, dP2, dP3, dN1, dLTRPNCa, dHTRPNCa]';
else
    output=[INa INaL Ito ICaL IKr IKs IK1 INaCa_i INaCa_ss INaK_cortassa_scalar  IKb INab ICab IpCa_cortassa_scalar Jdiff JdiffNa JdiffK Jup_cortassa_scalar Jleak Jtr Jrel CaMKa Istim, fINap, ...
        fINaLp, fICaLp, fJrelp, fJupp, cajsr, cansr, PhiCaL_ss, v, ICaL_i, I_ClCa, I_Clbk, ICaL_tot, force_norm, force, VNO];
end
...
function INaK_cortassa_scalar = getINaK_ORd2011_cortassascalar(v, F, R, T, nai, nao, ki, ko, ATPi, ADP, celltype, INaK_Multiplier)
%calculate INaK
zna=1.0;
k1p=949.5;
k1m=182.4;
...
%%%%%%%%%%%%%%%% adding INaK scalar f^ATP_NaK from A35 %%%%%%%%%%%%%%%
K1ATP_NaK = 8.0e-3; % mM ATP half saturation constant for NaK pump
KiADP_NaK = 0.1; % mM ADP inhibition constant for NaK pump

fNaK_ATP = 1 / (1 + ((K1ATP_NaK/ATPi) * (1 + (ADP / KiADP_NaK))));

INaK_cortassa_scalar = INaK * fNaK_ATP;
...
%% Jup Cortassa scalar added
function [Jup_cortassa_scalar, Jleak] = getJup_ORd2011_cortassascalar(cai, cansr, fJupp, ATPi, ADP, celltype, Jup_Multiplier)
...
%calculate serca pump, ca uptake flux - need to change this
...
%%%%%%%%%%%%%%%%%%%%%%%% adding Jup scalar f^serca_ATP from A47 %%%%%%%%%

Kmup = 0.01; % mM ATP half saturation constant for serca
Kiup = 0.14; % mM ADP first inhibition constant for serca
Kiprime_up = 5.1; % mM ADP second inhibition constant for serca

fserca_ATP = 1 / ((Kmup / ATPi) * (1 + (ADP / Kiup)) + ( 1+ (ADP / Kiprime_up)));

Jup_cortassa_scalar = Jup * fserca_ATP;

if Jup_cortassa_scalar < 0
    Jup_cortassa_scalar = 0;
end
